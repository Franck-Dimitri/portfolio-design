// resources/js/Components/Header.jsx (ou Layout/Header.jsx)
...
export default function Header() {
    // ✅ usePage() doit être appelé à l'intérieur du composant
    const { url } = usePage()
    ...
    const isActive = (href) => {
        if (href === '/') {
            return url === '/'  // Accueil exact
        }
        return url.startsWith(href)  // Pour les autres routes
    }
            ...
            <header ref={navRef} className={`navbar transition-all duration-300 ${scrolled ? 'scrolled shadow-dark-sm' : ''}`}>
                <div className="container-main h-full px-4 sm:px-6 flex items-center justify-between gap-8">
                    ...
                    <nav className='hidden md:flex items-center gap-1' aria-label="navigation principale">
                        {NAV_LINKS.map((link) => {
                            const active = isActive(link.href)  // ✅ Stocke le booléen
                            return (
                                <Link 
                                    key={link.href} 
                                    href={link.href} 
                                ...
                                >
                                    {link.label}
                                    {active && (  // ✅ Utilise active (booléen) au lieu de isActive (fonction)
                    ...
                    <div className='flex items-center gap-2'>
                        <ThemeSelector />
                        <button
                            className='md:hidden btn-ghost p-2 rounded-lg'  // ✅ btn-ghost corrigé, p-2 au lieu de p-s
